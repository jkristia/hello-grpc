...
export type HelloServerService = typeof HelloServerService;
export const HelloServerService = {
  sayHello: {
...
export interface HelloServerServer extends UntypedServiceImplementation {
  sayHello: handleUnaryCall<HelloRequest, HelloReply>;
}

export interface HelloServerClient extends Client {
  sayHello(
...
export const HelloServerClient = makeGenericClientConstructor(
  HelloServerService,
  "helloworld.HelloServer",
) as unknown as {
  new (address: string, credentials: ChannelCredentials, options?: Partial<ClientOptions>): HelloServerClient;
  service: typeof HelloServerService;
  serviceName: string;
};
